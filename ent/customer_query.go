...
func (cq *CustomerQuery) QueryRoles() *RoleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, customer.RolesTable, customer.RolesColumn),
		)
...
func (cq *CustomerQuery) WithRoles(opts ...func(*RoleQuery)) *CustomerQuery {
	query := (&RoleClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withRoles; query != nil {
		if err := cq.loadRoles(ctx, query, nodes, nil,
			func(n *Customer, e *Role) { n.Edges.Roles = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CustomerQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Role)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Customer)
	for i := range nodes {
		if nodes[i].role_customers == nil {
			continue
		}
		fk := *nodes[i].role_customers
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "role_customers" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
