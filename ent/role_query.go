...
func (rq *RoleQuery) QueryCustomers() *CustomerQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, role.CustomersTable, role.CustomersColumn),
		)
...
func (rq *RoleQuery) WithCustomers(opts ...func(*CustomerQuery)) *RoleQuery {
	query := (&CustomerClient{config: rq.config}).Query()
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withCustomers; query != nil {
		if err := rq.loadCustomers(ctx, query, nodes,
			func(n *Role) { n.Edges.Customers = []*Customer{} },
			func(n *Role, e *Customer) { n.Edges.Customers = append(n.Edges.Customers, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedCustomers {
		if err := rq.loadCustomers(ctx, query, nodes,
			func(n *Role) { n.appendNamedCustomers(name) },
			func(n *Role, e *Customer) { n.appendNamedCustomers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadCustomers(ctx context.Context, query *CustomerQuery, nodes []*Role, init func(*Role), assign func(*Role, *Customer)) error {
	...
	query.Where(predicate.Customer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.CustomersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.role_customers
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_customers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_customers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (rq *RoleQuery) WithNamedCustomers(name string, opts ...func(*CustomerQuery)) *RoleQuery {
	query := (&CustomerClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedCustomers == nil {
		rq.withNamedCustomers = make(map[string]*CustomerQuery)
	}
	rq.withNamedCustomers[name] = query
	return rq
}
