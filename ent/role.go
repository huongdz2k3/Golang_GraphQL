...
func (e RoleEdges) CustomersOrErr() ([]*Customer, error) {
	if e.loadedTypes[0] {
...
// QueryCustomers queries the "customers" edge of the Role entity.
func (r *Role) QueryCustomers() *CustomerQuery {
	return NewRoleClient(r.config).QueryCustomers(r)
}
...
func (r *Role) NamedCustomers(name string) ([]*Customer, error) {
	if r.Edges.namedCustomers == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := r.Edges.namedCustomers[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (r *Role) appendNamedCustomers(name string, edges ...*Customer) {
	if r.Edges.namedCustomers == nil {
		r.Edges.namedCustomers = make(map[string][]*Customer)
	}
	if len(edges) == 0 {
		r.Edges.namedCustomers[name] = []*Customer{}
	} else {
		r.Edges.namedCustomers[name] = append(r.Edges.namedCustomers[name], edges...)
	}
}
